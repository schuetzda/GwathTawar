...
namespace gwa
{
	...
	VulkanRenderPass::VulkanRenderPass(VkDevice logicalDevice, VkPhysicalDevice physicalDevice, VkFormat swapchainImageFormat, const gwa::renderer::RenderGraphDescription& description)
	{
		for (auto const& [keyRenderPass, pass] : description.passes)
		{
			std::vector<VkAttachmentDescription> attDescriptions;
			attDescriptions.resize(pass.colorAttachmentsCount);
			std::vector<VkAttachmentReference> attReferences;
			attReferences.resize(pass.colorAttachmentsCount);
			...
			const bool depthBufferingEnabled = pass.depthStencilAttachment < description.attachments.size();
			VkAttachmentReference depthAttachmentReference = {};

			if (depthBufferingEnabled)
			{
				const renderer::Attachment& depthAttachment = description.attachments.at(pass.depthStencilAttachment);
